...
{   
    ...
    scrollctrl->signal_scroll().connect(sigc::mem_fun<bool>(*this, &CairoGraph::on_scroll_event), true);
...
void CGraph::CairoGraph::set_graph_size(double x, double y, double w, double h) {
    offset_x = x;
    offset_y = y;
    graph_width = w;
    graph_height = h;
...
{
    if(!numpoints.size()) return; //SL, do not draw if nothing
    ...
    for (size_t j = 0; j < numpoints.size(); ++j)
    {
        ...
        if (CairoGraphLineStyle::DOTS == serieslinestyle[j])
        ...
        for (size_t i = 1; i < numpoints[j]; ++i)
        {
            ...
            if (CairoGraphLineStyle::SOLID_LINE == serieslinestyle[j] || CairoGraphLineStyle::DASHED_LINE == serieslinestyle[j])
            {
                cr->line_to(x, y);
            }
...
void CGraph::CairoGraph::set_visible(bool visible) {
    Gtk::DrawingArea::set_visible(visible);
