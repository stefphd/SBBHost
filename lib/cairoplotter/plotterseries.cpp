...
void CGraph::CairoGraph::add_point(size_t seriesnum, const double x, const double y, bool update_xminmax, bool update_yminmax)
{
    ...
    if (xlen > 0)
    {
        int i = seriesx[seriesnum].size() - (xlen + 1);
        if (seriesx[seriesnum].size() > xlen) seriesx[seriesnum].erase(seriesx[seriesnum].begin(), seriesx[seriesnum].begin() + i);
        if (seriesy[seriesnum].size() > xlen) seriesy[seriesnum].erase(seriesy[seriesnum].begin(), seriesy[seriesnum].begin() + i);
    }
    ...
    if ( true == update_xminmax)
    {
        if ( x < xmin) xmin = x;
        if ( x > xmax) xmax = x;
    }

    if ( true == update_yminmax)
    {
        if (y < ymin) {
            ymin = y;
            ymin -= 0.1 * (ymax - ymin);
...
void CGraph::CairoGraph::update_ylim(double y1, double y2) {
    bool has_found = false;
    reset_ylim();
    for (int i = 0; i < seriesy.size(); i++) {
        if (seriesy[i].size() != 0) {
            std::pair yminmax = std::minmax_element(std::begin(seriesy[i]), std::end(seriesy[i]));
    ...
    if (!has_found) ymin = y1, ymax = y2;
}

void CGraph::CairoGraph::reset_limits() 
{
    xmax = ymax = std::numeric_limits<double>::min();
    xmin = ymin = std::numeric_limits<double>::max();
}

void CGraph::CairoGraph::reset_xlim()
{
    xmax = std::numeric_limits<double>::min();
    xmin = std::numeric_limits<double>::max();
}

void CGraph::CairoGraph::reset_ylim()
{
    ymax = std::numeric_limits<double>::min();
    ymin = std::numeric_limits<double>::max();
}

void CGraph::CairoGraph::clear_series(size_t seriesnum) {
    if (seriesx[seriesnum].size() == 0) { return; }
    seriesx[seriesnum].clear();
    seriesy[seriesnum].clear();
    numpoints[seriesnum] = 0;
}

size_t CGraph::CairoGraph::get_num_series() {
    return numpoints.size();
}

void CGraph::CairoGraph::set_xwindow(int len) {
    xlen = len;
}

void CGraph::CairoGraph::set_xlim(double x1, double x2) {

    //set the x axis limits [xmin=x1, xmax=x2]

    xmin = x1;
    xmax = x2;
}

void CGraph::CairoGraph::set_ylim(double y1, double y2) {

    //set the y axis limits [ymin=y1, ymax=y2]
