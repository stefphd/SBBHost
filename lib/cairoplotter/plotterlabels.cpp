...
void CGraph::CairoGraph::create_tickmark_labels(const Cairo::RefPtr<Cairo::Context> &cr)
{
    ...
    ? font.set_size(static_cast<int>(4.0 * PANGO_SCALE * tick_scale * w / start_width))
    : font.set_size(tick_scale * PANGO_SCALE * 15);
...
{
    if (w > 768 && h > 768)
        font->set_size(static_cast<int>(scale * 21 * PANGO_SCALE * label_scale));
    else if (w > 640 && h > 640)
        font->set_size(static_cast<int>(scale * 17 * PANGO_SCALE * label_scale));
    else if (w > 256 && h > 256)
        font->set_size(static_cast<int>(scale * 14 * PANGO_SCALE * label_scale));
    else
        font->set_size(static_cast<int>(scale * 8 * PANGO_SCALE * label_scale));
}

void CGraph::CairoGraph::set_label_scale(double sc) {
    label_scale = sc;
}

void CGraph::CairoGraph::set_tick_scale(double sc) {
    tick_scale = sc;
}